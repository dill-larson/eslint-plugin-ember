...
/**
 * Hash of property types and their corresponding classifications,
 * so we can display them in an easy-to-read error message
 */
const NAMES = {
...
function determinePropertyType(
  ...
  if (parentType === 'component') {
    if (
      ...
      node.value !== null &&
      ember.isComponentLifecycleHook(node)
    ) {
  ...
  if (parentType !== 'model') {
    if (ember.isActionsProp(node)) {
      return 'actions';
    } else if (ember.isActionMethod(node)) {
      return 'action';
    }
  }
