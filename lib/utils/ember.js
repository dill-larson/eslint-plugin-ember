...
function isPropOfType(node, type) {
  if (types.isProperty(node) && types.isCallExpression(node.value)) {
    // Prop on classic class
    const calleeNode = node.value.callee;
    return types.isIdentifier(calleeNode) && calleeNode.name === type;
  } else if (types.isClassPropertyOrPropertyDefinition(node) || types.isMethodDefinition(node)) {
    // Prop on native class
    return decoratorUtils.hasDecorator(node, type);
  }
  return false;
}
...
function isActionFn(property) {
  if (property?.type === 'MethodDefinition' && property.decorators) {
    return property.decorators.some(({ expression }) => expression.name === 'action');
  }
  return false;
}

function isSingleLineFn(property, importedEmberName, importedObserverName) {
  const isSingleLineAccessor = utils.isPropAccessor(property) && utils.getSize(property) === 1;
  const isSingleLineComputed =
    decoratorUtils.hasDecorator(property, 'computed') && utils.getSize(property) === 1;
  const isSingleLineCallExpression =
    property.value &&
  ...
  return isSingleLineAccessor || isSingleLineComputed || isSingleLineCallExpression;
}

function isMultiLineFn(property, importedEmberName, importedObserverName) {
  const isMultiLineAccessor = utils.isPropAccessor(property) && utils.getSize(property) > 1;
  const isMultiLineComputed =
    decoratorUtils.hasDecorator(property, 'computed') && utils.getSize(property) > 1;
  const isMultiLineCallExpression =
    property.value &&
    types.isCallExpression(property.value) &&
    utils.getSize(property.value) > 1 &&
    !isObserverProp(property, importedEmberName, importedObserverName) &&
    (isComputedProp(property.value, importedEmberName, 'computed', { includeSuffix: true }) ||
      !types.isCallWithFunctionExpression(property.value));
  return isMultiLineAccessor || isMultiLineComputed || isMultiLineCallExpression;
}
