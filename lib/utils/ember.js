...
function isComputedProp(
  ...
  return (
    ...
    // @computed()
    decoratorUtils.hasDecorator(node, 'computed')
  );
}
...
function isSingleLineAccessor(property) {
  return (
    (utils.isPropAccessor(property) || isComputedProp(property)) && utils.getSize(property) === 1
  );
}

function isSingleLineFn(property, importedEmberName, importedObserverName) {
  return (
    isSingleLineAccessor(property) ||
    (property.value &&
